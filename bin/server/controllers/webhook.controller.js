...
import { GH_RELEASE_REG_EXP } from "../../utils.js";
const handleGithubWebhook = async (req, res) => {
    ...
    switch (event) {
        ...
        default:
            ...
            res.status(200).json({
                message: "Webhook received, but event is not included in webhook's allowed actions",
            });
    }
};
const handleRepository = async (req, res) => {
    try {
...
const createCommit = async (req, res) => {
    try {
        ...
        const isDefaultBranch = payload.pull_request.base.ref.replace("refs/heads/", "") ===
...
const createWorkflowDeployment = async (req, res) => {
    try {
        const payload = req.body;
        const [owner, repo] = payload.repository.full_name.split("/");
        const repository = await findRepository(owner, repo);
        ...
        const isDefaultBranch = payload.workflow_run.head_branch === repository.default_branch;
        ...
        const commit = await findCommit(repository, payload.workflow_run.head_commit.id);
...
const findRepository = async (owner, repo) => {
    const repository = await RepositoryModel.findOne({
        owner: { $eq: owner },
        name: { $eq: repo },
    });
...
const findCommit = async (repository, sha) => {
...
const verifyWebhookSignature = (signature, payload) => {
    if (!signature || !env.GH_WEBHOOK_SECRET)
        return false;
    ...
    try {
        ...
        const versionPromises = payload.target.map(async (release) => {
            try {
                ...
                const { data: { created_at: tagCreationDate }, } = await octokit.request("GET /repos/{owner}/{repo}/releases/tags/{tag}", {
                    owner,
                    repo,
                    tag: tagName,
                });
                ...
                if (!prevTag) {
                    ...
                    await DeploymentModel.create({
                        ...
                        finished_at: payload.environment === "prod"
                            ? payload.timestamp
                            : tagCreationDate,
                    });
                }
                    ...
                    const { data: { commits }, } = await octokit.request("GET /repos/{owner}/{repo}/compare/{base}...{head}", {
                        owner,
                        repo,
