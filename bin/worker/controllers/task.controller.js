import { CommitModel, DeploymentModel, RepositoryModel, } from "../../models/index.js";
import { GoogleDocumentClient } from "../../services/google.js";
import octokit from "../../services/octokit.js";
import { GH_RELEASE_REG_EXP, PROD_REG_EXP, UAT_REG_EXP } from "../../utils.js";
const scanRepository = async (client, owner, name) => {
...
const scanCommits = async (client, repository) => {
    ...
    const commitPromises = commits.map(async (commit) => {
        try {
            ...
            if (!cmt) {
                ...
                // No Date = skip
                if (!created_at) {
                    console.warn(`Skipping commit ${commit.sha} due to missing required fields`);
                    return null;
                }
...
const scanWorkflows = async (client, repository, commits, opts = {}) => {
    if (!commits?.length) {
    ...
    const deploymentPromises = filteredRuns.map(async (run) => {
        ...
        try {
            ...
            console.error(`[Worker]: Error processing deployment for run ${run.name}:`, error);
            return null;
        }
    });
    if (opts.return) {
        const deployments = (await Promise.all(deploymentPromises)).filter((deployment) => deployment !== null);
        return deployments;
    }
};
const scanReleases = async (client, repository, release) => {
    const tags = await client.paginate("GET /repos/{owner}/{repo}/tags", {
        owner: repository.owner,
        repo: repository.name,
    });
    const currTagIdx = tags.findIndex((tag) => tag.name === release.tagName);
    const { data: { created_at: tagCreationDate }, } = await client.request("GET /repos/{owner}/{repo}/releases/tags/{tag}", {
        owner: repository.owner,
        repo: repository.name,
        tag: release.tagName,
    });
    if (currTagIdx === -1) {
        console.warn(`Current tag (${release.tagName}) not found for ${repository.full_name}.`);
        return;
    }
    ...
    if (!prevTag) {
        ...
        if (!deployment) {
            await DeploymentModel.create({
                ...
                finished_at: release.environment === "prod" ? release.timestamp : tagCreationDate,
            });
        }
    }
    else {
        const { data: { commits }, } = await client.request("GET /repos/{owner}/{repo}/compare/{base}...{head}", {
            owner: repository.owner,
            repo: repository.name,
            ...
            try {
                ...
                if (!deployment) {
                    await DeploymentModel.create({
                        ...
                        finished_at: release.environment === "prod"
                            ? release.timestamp
                            : tagCreationDate,
                    });
                }
...
const parseDeployment = (deployment, environment) => {
    ...
    if (versionIndex === -1) {
        console.warn(`"Version" marker not found in deployment for ${deploymentVersion}.`);
        return null;
    }
...
const scanDevEnv = async (job) => {
    ...
    const repository = await scanRepository(octokit, owner, repo);
...
const scanUatProdEnv = async (job) => {
    ...
    if (!deployments || deployments.length === 0) {
        throw new Error("This document is either empty or worker failed to fetch the document");
    }
    const promises = deployments.map(async (deployment) => {
        ...
        const { deploymentVersion, content, selectedRepositories, timestamp } = parsed;
        await Promise.all(selectedRepositories.map(async (release) => {
            try {
